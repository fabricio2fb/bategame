...
export default function PartidaPage() {
  ...
  const [finalScores, setFinalScores] = useState<Array<{ playerId: string; name: string; avatarUrl?: string; score: number }>>([]);
  ...
  useEffect(() => {
    ...
    sock.on('score:updated', (data: { scores: Array<{ playerId: string; name: string; avatarUrl?: string; score: number }> }) => {
      setScores(data.scores);
    });

    sock.on('game:finished', (data: { scores: Array<{ playerId: string; name: string; avatarUrl?: string; score: number }> }) => {
      setIsFinished(true);
  ...
  if (roomNotFound) {
    return <RoomNotFoundState gameType="bateprimeiro" />;
  }
  ...
  return (
    <div className="h-[100dvh] lg:h-screen bg-gradient-to-br from-[#0c1929] via-[#0f2744] to-[#0a1628] flex flex-col overflow-hidden">
      ...
      <GameHeader
        ...
        isHost={isHost}
        players={scoresForPanel.map(p => ({ id: p.id, name: p.name, avatarUrl: p.avatarUrl, score: p.score }))}
              ...
              {scoresForPanel.map(p => (
                <div key={p.id} className="flex items-center justify-between rounded-xl bg-[#F8FAFC] border border-[#CBD5E1] px-3 py-2 text-sm">
                  <span className="flex min-w-0 items-center gap-2 font-semibold text-[#0F172A]">
                    <PlayerAvatar name={p.name} avatarUrl={p.avatarUrl} className="h-7 w-7" textClassName="text-[10px]" />
